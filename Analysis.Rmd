...
```
We noted in table 4 that age is not normally distributed and for this reason we cannot use the mean or average to compare groups, but we use the median. We use the Kruskal-Wallis Test, which is a non parametric test, to compare the medians of the two groups. 
...
Computed was the p-value (P < 0.0001) from the Kruskal-Wallis test. Also, the 95% Confidence interval for the difference in median age between the two gender groups (Male = m and Female = f) was calculated as [`r round(W_test$conf.int[[1]],2)`, `r round(W_test$conf.int[[2]],2)`]. These results can also be represented with the results of other numerical variables in a table as follows.
...
If the numerical variable has a symmetric distribution, results will be summarised and compared as below.
